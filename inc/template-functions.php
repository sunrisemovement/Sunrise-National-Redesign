...
function ea_scheduleCronJobs() {
	if ( !wp_next_scheduled( 'ea_cron_deleteOldEvents' ) ) {
		write_log("scheduled ea_cron_deleteOldEvents");
		wp_schedule_event(time(), 'hourly', 'ea_cron_deleteOldEvents');
	}
	if ( !wp_next_scheduled( 'ea_cron_fetchActions' ) ) {
		write_log("scheduled ea_cron_fetchActions");
		wp_schedule_event(time(), 'hourly', 'ea_cron_fetchActions');
	}
}
ea_scheduleCronJobs();
...
function ea_deleteOldEvents() {
	$calledDate = new DateTime("now");
	write_log($calledDate->format('d/m/Y')."-"."ea_deleteOldEvents called");
	$posts = get_posts([
		'numberposts' => '-1', // -1 for all posts
	  	'post_type' => 'events',
	  	'post_status' => 'publish',
	...
	foreach($posts as $postId) {
		...
		if($deletePost) {
			...
			write_log($calledDate->format('d/m/Y')."-"."ea_deleteOldEvents deleting ".$fileName);
			if(file_exists($fileName)) {
				unlink($fileName);
			}
		}
...
// 	}
// }
add_action('ea_cron_fetchActions', 'fetchNewOnlineActions');
add_action('ea_cron_deleteOldEvents', 'ea_deleteOldEvents');