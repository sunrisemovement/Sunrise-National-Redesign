...
/*	-----------------------------------------------------------------------------------------------
	Namespace
--------------------------------------------------------------------------------------------------- */
var sunrisenational = sunrisenational || {}; // Set a default value for scrolled.

sunrisenational.scrolled = 0; // polyfill closest
...
sunrisenational.createEvent = function (eventName) {
  var event;
  ...
  hideAndShowModals: function hideAndShowModals() {
    ...
    modals.forEach(function (modal) {
      modal.addEventListener("toggle-target-before-inactive", function (event) {
        ...
        _win.sunrisenational.scrolled = parseInt(styles.top, 10);
      ...
      modal.addEventListener("toggle-target-after-inactive", function (event) {
        ...
        setTimeout(function () {
          var clickedEl = sunrisenational.toggles.clickedEl;
          ...
          _win.scrollTo(0, Math.abs(_win.sunrisenational.scrolled + getAdminBarHeight()));

          _win.sunrisenational.scrolled = 0;
        }, 500);
      });
    });
  },
...
sunrisenational.intrinsicRatioVideos = {
  init: function init() {
    this.makeFit();
...
}; // sunrisenational.instrinsicRatioVideos

/*	-----------------------------------------------------------------------------------------------
	Modal Menu
--------------------------------------------------------------------------------------------------- */

sunrisenational.modalMenu = {
  init: function init() {
  ...
  expandLevel: function expandLevel() {
    ...
    modalMenus.forEach(function (modalMenu) {
      ...
      if (activeMenuItem) {
        sunrisenationalFindParents(activeMenuItem, "li").forEach(function (element) {
          var subMenuToggle = element.querySelector(".sub-menu-toggle");

          if (subMenuToggle) {
            sunrisenational.toggles.performToggle(subMenuToggle, true);
          }
        });
      }
    });
  ...
  keepFocusInModal: function keepFocusInModal() {
    ...
    _doc.addEventListener("keydown", function (event) {
      var toggleTarget,
          ...
          tabKey,
          shiftKey,
          clickedEl = sunrisenational.toggles.clickedEl;
...
sunrisenational.toggles = {
  ...
  performToggle: function performToggle(element, instantly) {
    ...
    if (target.classList.contains(activeClass)) {
      target.dispatchEvent(sunrisenational.createEvent("toggle-target-before-active"));
    } else {
      target.dispatchEvent(sunrisenational.createEvent("toggle-target-before-inactive"));
    ...
    setTimeout(function () {
      ...
      if (toggle.dataset.toggleType === "slidetoggle" && !instantly && duration !== "0") {
        sunrisenationalMenuToggle(newTarget, duration);
      } else {
      ...
      sunrisenationalToggleAttribute(toggle, "aria-expanded", "true", "false");

      if (self.clickedEl && -1 !== toggle.getAttribute("class").indexOf("close-")) {
        sunrisenationalToggleAttribute(self.clickedEl, "aria-expanded", "true", "false");
      } // Toggle body class.
      ...
      target.dispatchEvent(sunrisenational.createEvent("toggled")); // Trigger events on the toggle targets after they are toggled.

      if (target.classList.contains(activeClass)) {
        target.dispatchEvent(sunrisenational.createEvent("toggle-target-after-active"));
      } else {
        target.dispatchEvent(sunrisenational.createEvent("toggle-target-after-inactive"));
      }
    }, timeOutTime);
  },
  // Do the toggle.
...
function sunrisenationalDomReady(fn) {
...
sunrisenationalDomReady(function () {
  sunrisenational.toggles.init(); // Handle toggles.

  sunrisenational.coverModals.init(); // Handle cover modals.

  sunrisenational.intrinsicRatioVideos.init(); // Retain aspect ratio of videos on window resize.

  sunrisenational.modalMenu.init(); // Modal Menu.

  sunrisenational.primaryMenu.init(); // Primary Menu.

  sunrisenational.touchEnabled.init(); // Add class to body if device is touch-enabled.
});
/*	-----------------------------------------------------------------------------------------------
	Helper functions
--------------------------------------------------------------------------------------------------- */
...
function sunrisenationalToggleAttribute(element, attribute, trueVal, falseVal) {
...
function sunrisenationalMenuToggle(target, duration) {
...
function sunrisenationalFindParents(target, query) {
